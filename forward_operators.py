...
Includes the eight SGPS task operators and their required numerical helpers.
"""
...
class DeterministicReflectionPad2d(nn.Module):
    """Legacy revision's slice/flip reflection, with deterministic CUDA backward."""
    ...
    def forward(self, value):
        ...
        if size == 0:
            return value
        left = value[..., 1:size + 1].flip(-1)
        right = value[..., -size - 1:-1].flip(-1)
        value = torch.cat((left, value, right), dim=-1)
        top = value[..., 1:size + 1, :].flip(-2)
        bottom = value[..., -size - 1:-1, :].flip(-2)
        return torch.cat((top, value, bottom), dim=-2)


def use_deterministic_padding(operator):
    if isinstance(operator, (GaussianBlur, MotionBlur)):
        padding = operator.conv.seq[0]
        if isinstance(padding, nn.ReflectionPad2d):
            sides = padding.padding
            if isinstance(sides, tuple):
                if len(set(sides)) != 1:
                    raise ValueError('Expected symmetric reflection padding')
                sides = sides[0]
            operator.conv.seq[0] = DeterministicReflectionPad2d(sides)
...
class NonlinearBlur(Operator):
    ...
    def prepare_nonlinear_blur_model(self, opt_yml_path):
        ...
        # Fall back to the standard checkpoint directories if needed.
        ...
        if not model_path.exists():
            raise FileNotFoundError(
                ...
                f"or place '{model_path.name}' under checkpoints/."
            )
